...
class QueryExecutor:
    ...
    async def execute(self, sql: str, params: tuple = ()) -> list[dict[str, Any]]:
        return await run_blocking(self._execute_sync, sql, params)
...
class AssetSearchService:
    ...
    async def search(
        ...
        results, timings = await run_blocking(
...
class ChatSessionService:
    ...
    def _ensure_session_sync(
        self, session_id: str, title: str | None, metadata: dict[str, Any]
    ) -> None:
        pool_instance = get_pool_sync(self._settings)
        conn = pool_instance.getconn()
        try:
    ...
    async def ensure_session(
        ...
        await run_blocking(self._ensure_session_sync, session_id, title, metadata or {})
    ...
    async def append_message(
        ...
        await run_blocking(self._append_message_sync, session_id, role, content, metadata or {})
    ...
    async def recent_messages(self, session_id: str, limit: int = 10) -> list[dict[str, Any]]:
        return await run_blocking(self._recent_messages_sync, session_id, limit)
    ...
    async def list_sessions(self, limit: int = 25) -> list[dict[str, Any]]:
        return await run_blocking(self._list_sessions_sync, limit)
